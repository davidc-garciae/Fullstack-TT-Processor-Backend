...
export default registerAs('processor', () => ({
  baseUrl:
    ...
    'https://api-sandbox.co.uat.processor.dev/v1',
  ...
  transactionByIdPath:
    process.env.PAYMENT_PROCESSOR_TRANSACTION_BY_ID_PATH ??
    '/transactions/{{id}}',
  /** Poll when status is PENDING: interval ms and max attempts (total wait = interval * maxAttempts). */
  pollIntervalMs:
    Number(process.env.PAYMENT_PROCESSOR_POLL_INTERVAL_MS) || 2000,
  pollMaxAttempts:
    Number(process.env.PAYMENT_PROCESSOR_POLL_MAX_ATTEMPTS) || 15,
}));
